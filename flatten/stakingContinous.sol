// Dependency file: contracts\timelib.sol
...
// Root file: contracts\stakingContinous.sol

pragma solidity 0.8.3;
pragma experimental ABIEncoderV2;

// import "contracts\timelib.sol";
// import "contracts\math.sol";
// import "D:\Git\hardhat-framework\node_modules\@openzeppelin\contracts\token\ERC20\IERC20.sol";

contract lunaStableCoinFund {
    struct stakeType {
        uint8 Type;
    ...
    uint48 currentStakedLunaAmount;
    uint48 totalProfitsDistrubuted;
    uint48 totalStakedLunaAmount;
    ...
    function getTotalStaked()
        external
        view
        returns (uint48 totalLunaAmountStaked)
    {
    ...
    function getCurrentStakedAmount()
        external
        view
        returns (uint48 currentStakedLuna)
    {
    ...
    function addStake(uint48 _amount, uint8 _type) external {
        ...
        currentStakeID += 1;
        //Transfer luna tokens from the msg.sender(stake owner) to the contract
    ...
    function withdraw(
        ...
        uint256 totalReturns = compound(
            stakeByID[_stakeID].lunaAmount,
            (stakeTypes[stakeByID[_stakeID].Type].percentageReturn * (10**8)),
            periods
        );

        uint256 stakeReturns = totalReturns - stakeByID[_stakeID].lunaAmount;
        ...
        if (
            ...
            // Transfer the luna tokens to the stake owner.
            lunaToken.approve(
                address(this),
                stakeByID[_stakeID].settlementAmount
            );
    ...
    function settleStakes(uint32[] memory _stakeIDs) external onlyOwner {
        for (uint256 i = 0; i < _stakeIDs.length; i++) {
            if (
                ...
                currentStakedLunaAmount -= stakeByID[_stakeIDs[i]]
                    .settlementAmount;
