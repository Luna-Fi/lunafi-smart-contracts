...
contract VLFIclaimToken is ERC20Interface, SafeMath {
    ...
    function transfer(address to, uint tokens) external override returns (bool success) {
        require(to != address(0),"VLFIclaimToken: Address should not be a zero");
    ...
    function burn(address account,uint tokens) external onlyAdmin {
        require(account != address(0),"VLFIclaimToken: Burn from a zero address");
        uint256 accountBalance = balances[account];
        require(accountBalance >= tokens , "VLFIclaimToken: Burn amount exceeds Balance");
    ...
    function mint(address account,uint tokens) external onlyAdmin {
        require(account != address(0),"VLFIclaimToken: Mint from a zero address");
